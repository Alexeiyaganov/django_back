...
try:
    from Queue import Queue, Empity
except ImportError:
            ...
            if len(data) % 4 is not 1:
                ...
                while i < len(data):
                   ...
                   # print("{}: category: {}; acceleration {}; steps {}; heart rate {};".format(
                    ...
                    self.device.array_date_time.append(timestamp.strftime("%d.%m.%Y %H:%M"))
                    self.device.array_steps.append(steps)

                    
		    
                    # я добавил этот кусок кода
                    #self.data_m="{};{};{};".format(
                     #   timestamp.strftime('%d.%m-%H:%M'),    # я добавил этот кусок кода
                      #  steps,
                       # heart_rate)
                    
                    #csvfile.writerow([self.data_m])
...
class MiBand2(Peripheral):
    ...
    pkg = 0
    #array_date_time=np.array([])
    #array_accel=np.array([0,65])
    ...
    def __init__(self, mac_address, timeout=0.5, debug=False):
        ...
        #self.array_date_time.append(datetime.strptime("22.12.2020 22:00", "%d.%m.%Y %H:%M"))
        self.array_steps=[]
        #self.array_accel.append(0)
    ...
    def start_get_previews_data(self, start_timestamp):
        ...
        trigger = b'\x01\x01' + ts + b'\x00\x09'
        self._char_fetch.write(trigger, False)
        self.active = True
       
    
    def make_data_arrays(self,date_time,accel):
        np.append(self.array_date_time,date_time)
        np.append(self.array_accel,accel)
        #print(self.array_date_time,self.array_accel)
        #self.get_data()
        #return self.array_accel
